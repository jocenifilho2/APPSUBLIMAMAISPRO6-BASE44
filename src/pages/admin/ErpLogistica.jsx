...
export default function ErpLogistica() {
  ...
  const avancarStatus = async (item) => {
    ...
    if (item._tipo === 'PRODUTO') {
        updatePedidoMutation.mutate({ id: item.id, data: { status: proximo } });
    } else {
        updateImpressaoMutation.mutate({ id: item.id, data: { status: proximo } });
    }
  ...
  return (
    <div className="p-4">
      <h2 className="text-xl font-bold mb-4">Painel Logístico</h2>
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {COLUMNS.map(col => (
          <Card key={col.key} className={`p-4 ${col.color}`}>
